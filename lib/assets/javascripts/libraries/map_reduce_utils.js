...
root.emitResult = root.emitResult || function(value) {
  if (value['IPP'] > 0) emit({"mrn" : value.medical_record_id,
                              "oid" : ObjectId()}, value);
}

root.map = function(record, population, denominator, numerator, exclusion, denexcep, msrpopl, observ, occurrenceId, isContinuousVariable, stratification) {
  ...
  if (isContinuousVariable) {
    value = calculateCV(record, population, msrpopl, observ, occurrenceId, value, stratification)
  } else {
